classdef NmpcControl < handle
    ...
    methods
        ...
        function [u, T_opt, X_opt, U_opt] = get_u(obj, x0, ref)
            ...
            % for d = 1:delay
            %     % Use the oldest control input in the memory for delay compensation
            %     u_delay = mem_u(:, end);
            %     k1 = obj.rocket.f(x_, u_delay);
            %     k2 = obj.rocket.f(x_ + obj.rocket.Ts/2 * k1, u_delay);
            %     k3 = obj.rocket.f(x_ + obj.rocket.Ts/2 * k2, u_delay);
            %     k4 = obj.rocket.f(x_ + obj.rocket.Ts * k3, u_delay);
            %     x_ = x_ + obj.rocket.Ts/6 * (k1 + 2*k2 + 2*k3 + k4);
            % end
